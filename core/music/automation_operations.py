...
def automation_write(
    ...
    *,
    ...
    def mutate(project: dict[str, Any]) -> dict[str, Any]:
        ...
        state["track_id"] = int(track_id)
        return project

    project = project_repository.update_project(mutate)
    saved_track = track_lookup.find_track(project, state["track_id"])
    summary = {
...
def automation_learned_parameter_upsert(
    ...
    def mutate(project: dict[str, Any]) -> dict[str, Any]:
        ...
        state["saved_item_id"] = saved_item["id"]
    ...
    saved = next(
        ...
        if item["id"] == state["saved_item_id"]
    )
    return project, {**deepcopy(saved), "created": state["created"]}
