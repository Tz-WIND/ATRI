...
def midi_write(
    ...
    *,
    ...
    synced_track = track_lookup.find_track(project, track_id)
    summary = {
        "track_id": track["id"],
...
def midi_batch_edit(
    ...
    *,
    ...
    project = (
        music_project.normalize_project(project)
        if dry_run
        else project_repository.save_project(project)
    )
    summary = {
        **changed,
        "selection": music_project._selection_summary(base_selection),
