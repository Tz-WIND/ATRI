  ...
  <div v-if="currentSong">
    ...
    >
      <Transition name="peek-fade">
        <div class="peek-pill" v-if="peekVisible" @click="collapsed = false; peekVisible = false">
    ...
    <Transition name="player-slide">
      <div class="music-player" v-show="!collapsed" @click.self="showFullPlayer = true">
        <!-- Collapse button -->
        <button class="collapse-btn" @click.stop="collapsed = true" title="Hide player bar">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
        </button>

        <!-- Progress bar -->
        <div class="player-progress-track" @click.stop="onProgressClick" ref="progressTrack">
          <div class="player-progress-fill" :style="{ width: progress + '%' }"></div>
        </div>

        <div class="player-inner">
          <!-- Left: cover + info -->
          <div class="player-left" @click="showFullPlayer = true">
            <div class="player-cover-wrap">
              ...
              <div class="player-cover placeholder" v-else>
                <svg viewBox="0 0 24 24" fill="currentColor" opacity="0.4"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55C7.79 13 6 14.79 6 17s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/></svg>
              </div>
            </div>
            <div class="player-info">
              <div class="player-song-name">{{ currentSong.title }}</div>
              <div class="player-artist">{{ currentSong.artist }}</div>
            </div>
            <div class="player-quality" v-if="currentSong.lossless">
              <span class="quality-badge" v-if="currentSong.bit_depth >= 24 || currentSong.sample_rate > 48000">Hi-Res</span>
              <span class="quality-badge" v-else>Lossless</span>
            </div>
          </div>
          ...
          <div class="player-right" @click.stop>
            ...
            <div class="volume-wrap">
              <button class="ctrl-btn small" @click="toggleMute">
                <svg v-if="volume > 50" viewBox="0 0 24 24" fill="currentColor"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>
                <svg v-else-if="volume > 0" viewBox="0 0 24 24" fill="currentColor"><path d="M18.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM5 9v6h4l5 5V4L9 9H5z"/></svg>
