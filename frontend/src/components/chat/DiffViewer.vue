<template>
  <div
    v-if="lines.length"
    class="diff-viewer"
  >
    <div class="diff-summary">
      <span class="diff-file">{{ displayName }}</span>
      <span class="diff-stat add">+{{ addCount }}</span>
      <span class="diff-stat del">-{{ delCount }}</span>
    </div>
    <div class="diff-lines">
      <div
        v-for="(line, i) in lines"
        :key="i"
        :class="['diff-line', line.cls]"
      >
        <span class="diff-gutter">{{ line.mark }}</span>
        <code>{{ line.text || ' ' }}</code>
      </div>
    </div>
  </div>
</template>

<script setup>
import { computed } from 'vue'

const props = defineProps({
  diff: { type: String, default: '' },
  raw: { type: String, default: '' },
  fileName: { type: String, default: '' },
})

const parsed = computed(() => {
  if (!props.diff) return { lines: [], addCount: 0, delCount: 0, fileName: '' }

  const rawLines = props.diff.split(/\r?\n/).filter((l, i, arr) => i < arr.length - 1 || l !== '')
  let fileName = props.fileName
  const lines = rawLines.map(line => {
    let cls = 'ctx', mark = ' ', text = line
    if (line.startsWith('+++') || line.startsWith('---')) {
      cls = 'file'
      if (!fileName && line.startsWith('+++ b/')) fileName = line.slice(6)
    } else if (line.startsWith('@@')) {
      cls = 'meta'
    } else if (line.startsWith('+')) {
      cls = 'add'; mark = '+'; text = line.slice(1)
    } else if (line.startsWith('-')) {
      cls = 'del'; mark = '-'; text = line.slice(1)
    } else if (line.startsWith(' ')) {
      text = line.slice(1)
    }
    return { cls, mark, text }
  })

  return {
    lines,
    addCount: lines.filter(l => l.cls === 'add').length,
    delCount: lines.filter(l => l.cls === 'del').length,
    fileName: fileName || (props.raw || '').split(/\r?\n/, 1)[0] || 'File diff',
  }
})

const lines = computed(() => parsed.value.lines)
const addCount = computed(() => parsed.value.addCount)
const delCount = computed(() => parsed.value.delCount)
const displayName = computed(() => parsed.value.fileName)
</script>

<style scoped>
.diff-viewer {
  background: rgba(24, 24, 24, 0.74);
  border: 1px solid var(--border);
  border-radius: 7px;
  max-height: 360px;
  overflow: auto;
  font-size: 11px;
  line-height: 1.48;
  box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.02);
}

.diff-summary {
  position: sticky;
  top: 0;
  z-index: 1;
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 7px 10px;
  border-bottom: 1px solid var(--border);
  color: var(--t3);
  background: rgba(24, 24, 24, 0.92);
  font-family: var(--mono);
  font-size: 11px;
}

.diff-file { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.diff-stat.add { color: var(--acc2); }
.diff-stat.del { color: var(--red); }

.diff-line {
  display: grid;
  grid-template-columns: 34px minmax(max-content, 1fr);
  min-width: 100%;
  font-family: var(--mono);
  white-space: pre;
}

.diff-line.add { background: var(--acc-bg); }
.diff-line.add .diff-gutter {
  color: var(--acc2); background: rgba(125, 168, 232, 0.18);
  border-right-color: rgba(125, 168, 232, 0.3);
}
.diff-line.add code { color: #dceaff; }

.diff-line.del { background: var(--red-bg); }
.diff-line.del .diff-gutter {
  color: var(--red); background: rgba(255, 141, 127, 0.18);
  border-right-color: rgba(255, 141, 127, 0.3);
}
.diff-line.del code { color: #ffe0dc; }

.diff-line.meta { background: rgba(125, 168, 232, 0.1); }
.diff-line.meta .diff-gutter {
  color: var(--acc2); background: rgba(125, 168, 232, 0.16);
  border-right-color: rgba(125, 168, 232, 0.26);
}
.diff-line.meta code { color: #bfd8ff; }

.diff-line.file { background: rgba(24, 24, 24, 0.86); }
.diff-line.file code { color: var(--t3); }

.diff-line code { display: block; padding: 1px 12px; color: var(--t2); font-family: inherit; }

.diff-gutter {
  padding: 1px 9px 1px 0;
  text-align: right;
  color: var(--t4);
  background: rgba(255, 255, 255, 0.035);
  border-right: 1px solid var(--border-light);
  user-select: none;
}
</style>
